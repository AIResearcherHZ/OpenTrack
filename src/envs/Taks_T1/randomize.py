...
# ==============================================================================
"""Utilities for randomization for Taks_T1."""
import jax
...
# Taks_T1 torso body id (torso_link)
TORSO_BODY_ID = 16
...
def domain_randomize_model(model: mjx.Model, rng: jax.Array):
    ...
    def rand_dynamics(rng):
        ...
        frictionloss = model.dof_frictionloss[6:] * jax.random.uniform(
            key, shape=(consts.NUM_JOINT,), minval=0.5, maxval=2.0
        )
        ...
        armature = model.dof_armature[6:] * jax.random.uniform(
            key, shape=(consts.NUM_JOINT,), minval=1.0, maxval=1.05
        )
        ...
        body_mass = model.body_mass.at[TORSO_BODY_ID].set(
            model.body_mass[TORSO_BODY_ID] * (1 + ratio)
        )
        body_inertia = model.body_inertia.at[TORSO_BODY_ID].set(
            model.body_inertia[TORSO_BODY_ID] * (1 + ratio)[..., None]
        )
        ...
        qpos0 = qpos0.at[7:].set(
            qpos0[7:] + jax.random.uniform(
                key, shape=(consts.NUM_JOINT,), minval=-0.05, maxval=0.05
            )
        )
...
def domain_randomize_terrain(
    ...
    def rand_dynamics(rng):
        ...
        # Randomize terrain
        rng, key = jax.random.split(rng)
        hfield_data = all_hfield_data[
            jax.random.randint(key, shape=(), minval=0, maxval=1024)
        ]
