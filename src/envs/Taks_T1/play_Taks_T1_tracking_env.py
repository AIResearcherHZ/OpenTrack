...
from src.envs.Taks_T1 import Taks_T1_tracking_constants as consts
...
class PlayTaks_T1TrackingEnv:
    ...
    def __init__(
        ...
        if self.use_renderer:
            ...
            self.writer = None
            os.makedirs(f"experiments/{self.exp_name}/videos", exist_ok=True)
    ...
    def _post_init(self):
        ...
        self.prepare_trajectory(self._config.reference_traj_config.name)
    ...
    def reset(self):
        ...
        if self.use_renderer:
            os.makedirs(
                f"experiments/{self.exp_name}/videos/{self.ref_traj_names[self.current_traj_info.traj_state.traj_no][0]}",
                exist_ok=True,
            )
            self.writer = imageio.get_writer(
                f"experiments/{self.exp_name}/videos/{self.ref_traj_names[self.current_traj_info.traj_state.traj_no][0]}/{self.ref_traj_names[self.current_traj_info.traj_state.traj_no][1]}.mp4",
                fps=50,
            )
    ...
    def _reset_from_current_traj(self):
        ...
        obs, history = self.get_obs(qpos, qvel, info)
    ...
    def prepare_trajectory(self, dataset_dict: Dict[str, List[str]]) -> Trajectory:
        ...
        for dataset_name, traj_names in dataset_dict.items():
            ...
            for t_name in traj_names:
                ...
                # load the npz file
                traj_path = os.path.join(path_to_datasets, "Taks_T1", f"{t_name}.npz")
        ...
        self.load_trajectory(trajectory, warn=False)
